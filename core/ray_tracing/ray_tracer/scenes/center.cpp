...
Scene *make_scene() {
        ...
        for (int J = -n; J <= n; J++) {
            for (int K = 0; K < 5; K++) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if ((I + J) % 2 == 0) {
                    primitives.push_back(new GeometricPrimitive(new Sphere(Transform::translate(I*15 + cos(theta)*r,K*2,J*15 + sin(theta)*r), 1),
                                         color_tex, NULL, 0, 1.5, 0.5));
                    } else {
                    primitives.push_back(new GeometricPrimitive(new Sphere(Transform::translate(I*15 + cos(theta)*r,K*2,J*15 + sin(theta)*r), 1),
                                         color_tex, NULL, 0.8));
                    }
    ...
    primitives.push_back(new GeometricPrimitive(new Plane(Point(0,-0.5,0), Vector(1,0,0), Vector(0,0,1), 1000, 1000),
                         new CheckerTexture(300,300,
                            new ConstantTextureRGB(RGB(1,1,1)),
                            new ConstantTextureRGB(RGB(0,0,0))
		         ),
                         NULL, 0.5));
    ...
    scene->add_light(new PointLight(Point(0,120,0), 7000.f*RGB(1,1,1)));
